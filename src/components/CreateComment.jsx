...
export default function CreateComment({id}) {
  ...
  const {isPending, data, mutate, } = useMutation({mutationFn:postComment,onSuccess:()=>{
    ...
    <>
      ...
      <div className="p-5">
        ...
        <div className="flex justify-end">
          <button onClick={handlePostComment} className="xl:hidden mt-3 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-2 px-4 rounded-lg transition duration-150 ease-in-out" disabled={isPending || !comment.trim()}>
            Post Comment
          </button>
        </div>
      </div>
